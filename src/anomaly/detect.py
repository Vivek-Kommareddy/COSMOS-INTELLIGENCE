...
def _analyse_metric(
    ...
    anomaly_dates_all = daily.loc[anomaly_mask, "date"]

    recent = daily.tail(lookback)
    prior  = daily.iloc[-lookback * 2:-lookback]

    recent_val = float(recent[metric].sum())
    ...
    anomaly_dates_recent = sorted(
        {d.strftime("%Y-%m-%d") for d in anomaly_dates_all
         if d.strftime("%Y-%m-%d") in recent_date_strs}
    )
