...
def transform_data(df: pd.DataFrame, processed_path: Path) -> pd.DataFrame:
    ...
    # 2. Parse dates ──────────────────────────────────────────────────────────
    before = len(clean)
    clean["date"] = pd.to_datetime(clean["date"], infer_datetime_format=True, errors="coerce")
    ...
    if clean.empty:
        raise ValueError(
            f"All {before} rows were dropped because no valid dates could be parsed. "
            "Please ensure your date column contains recognisable dates (e.g. 2024-01-15, 01/15/2024, Jan 15 2024)."
        )

    # 3. Coerce numerics ──────────────────────────────────────────────────────
    for col in NUMERIC_COLUMNS:
        before_coerce = clean[col].isna().sum()
        clean[col] = pd.to_numeric(clean[col], errors="coerce").fillna(0.0)
        coerced = int(clean[col].eq(0.0).sum()) - int(before_coerce)
        if coerced > 0:
            logger.warning(
                "Non-numeric values coerced to 0 in column",
                extra={"column": col, "count": coerced},
            )
    ...
    if grouped.empty:
        raise ValueError(
            "No data remained after grouping and aggregation. "
            "Please check that your file contains valid numeric revenue/orders values and parseable dates."
        )
