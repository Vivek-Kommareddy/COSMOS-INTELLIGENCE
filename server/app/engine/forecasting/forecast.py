...
def _linear_forecast(series: pd.Series, periods: int) -> dict[str, Any]:
    ...
    # Need at least 2 points to fit a line; fall back to flat forecast if not enough data
    if len(y) < 2:
        flat = float(y[0]) if len(y) == 1 else 0.0
        flat_list = [round(flat, 2)] * periods
        return {
            "forecast_values":  flat_list,
            "confidence_lower": flat_list,
            "confidence_upper": flat_list,
            "model":            "flat_fallback",
        }

    # Guard: NaN values in y (from coerced non-numeric data) cause polyfit to fail
...
def _forecast_one_metric(
    ...
    if daily.empty or len(daily) == 0:
        ...
        recent_avg = float(daily[metric].tail(14).mean())
    else:
        last_val = daily[metric].iloc[-1]
        recent_avg = float(last_val) if pd.notna(last_val) else 0.0
